...
def run_experiment(bs, n_epochs, lr1, lr2):
    ...
    #print(first_samp.shape)#get first sample in first batch
    ...
    class Conv_Pred(nn.Module):
        def __init__(self):
            ...
            self.fce1 = nn.Linear(512*2*2, 512*2)
            self.fce2 = nn.Linear(512*2, 45)
        ...
        def forward(self, x):
            ...
            #print(x.shape, self.fce1.weight.shape)
            
            x = F.relu(self.fce1(x))
            z = nn.Softmax(dim=1)(self.fce2(x.squeeze()))
